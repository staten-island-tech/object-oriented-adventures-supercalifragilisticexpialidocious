...
map = [['|' if i in [0] else '  ' for i in range(100)] for _ in range(25)]
for row in map: 
    for i in range(2):
        row[random.randint(0, 98)] = '🌲' 
...
while True: 
    ...
    if whereToGo == "W" and playerCoords[0] - 1 >= 0 and map[playerCoords[0] - 1][playerCoords[1]] != "#":
        map[playerCoords[0]][playerCoords[1]] = "  " 
        playerCoords[0] -= 1 
    elif whereToGo == "A" and map[playerCoords[0]][playerCoords[1] - 1] not in ["|", "#"]:
        map[playerCoords[0]][playerCoords[1]] = "  "
        playerCoords[1] -= 1
    elif whereToGo == "S" and playerCoords[0] + 1 < mapSize[0] and map[playerCoords[0] + 1][playerCoords[1]] != "#":
        map[playerCoords[0]][playerCoords[1]] = "  "
        playerCoords[0] += 1
